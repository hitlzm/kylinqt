...
Rectangle {
    ...
    RowLayout {
        ...
        Row {
            ...
            Rectangle { id: statusDot; width: 12; height: 12; radius: 6; anchors.verticalCenter: parent.verticalCenter
    ...
    // ========== 视频区域 ==========
    // MediaPlayer + VideoOutput 代替 VlcVideoPlayer —— 视频由 QML 原生渲染
    Rectangle {
        id: videoBorder
        anchors.left: parent.left;   anchors.leftMargin: 12
        anchors.right: parent.right; anchors.rightMargin: 12
    ...
    MediaPlayer {
        id: mediaPlayer
        autoPlay: false
        volume: 0.0   // 静音：音频由 C++ vlcplayer 统一管理
    }
    ...
    RowLayout {
        ...
        CusButton_Blue {
            ...
            onClicked: {
                ...
                if (vlcplayer.playing) { vlcplayer.pause(); mediaPlayer.pause() }
                else { vlcplayer.play(); mediaPlayer.play() }
            }
        }
        CusButton_Blue { text: "停止"; Layout.fillWidth: true; height: 40
            onClicked: { vlcplayer.stop(); mediaPlayer.stop() } }
        Text { text: "🔈"; font.pixelSize: 18; Layout.alignment: Qt.AlignVCenter }
        CusSlider { id: volumeSlider; Layout.preferredWidth: 120; showNumber: true; from: 0; to: 200; value: vlcplayer.volume
            onMoved: vlcplayer.setVolume(value) }
        CusLabel { text: Math.round(vlcplayer.volume / 2) + "%"; font.pixelSize: 14; Layout.preferredWidth: 45; horizontalAlignment: Text.AlignHCenter }
    }
    ...
    RowLayout {
        ...
        CusTextField { id: urlInput; Layout.fillWidth: true; text: vlcplayer.url.toString(); font.pixelSize: 14
    ...
    function connectToUrl(newUrl) { if (newUrl === "") return; vlcplayer.stop(); mediaPlayer.stop(); vlcplayer.openUrl(newUrl); mediaPlayer.source = newUrl; _connected = true }

    // ========== C++ vlcplayer 信号监听 ==========
    Connections { target: vlcplayer
        function onPlayingChanged() { console.log("vlcplayer playingChanged:", vlcplayer.playing) }
        function onEnded() { console.log("vlcplayer: 播放结束"); mediaPlayer.stop() }
        function onError(errorMsg) { console.log("vlcplayer error:", errorMsg); mediaPlayer.stop() }
        function onStopped() { console.log("vlcplayer: 已停止") }
    }
}
