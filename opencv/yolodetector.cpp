                               ...
                               std::vector<YoloDetection> &detections)
{
    ...
    // ── 遍历所有输出层（3 个尺度） ──   //三个层分别用于检测小尺度目标，中尺度目标，大尺度目标
    for (const auto &output : outputs) {
        ...
        for (int i = 0; i < output.rows; ++i, data += output.cols) {
            // 提取每个类别的置信度并找最大值
            cv::Mat scores = output.row(i).colRange(5, output.cols);   //提取类别分数
            cv::Point classIdPoint;
            double confidence;
            cv::minMaxLoc(scores, nullptr, &confidence, nullptr, &classIdPoint);
            // 如果确定只有 1 类，可以直接简化为（省去 minMaxLoc 的微小开销）：
            // int classId = 0;
            // float confidence = data[5];  // 直接取第 6 个值（即唯一的类别分数）
